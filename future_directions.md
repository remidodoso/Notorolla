...
we already use ([src/js/core/random.js](src/js/core/random.js), [src/js/core/transforms.js](src/js/core/transforms.js)).
...
- **Transforms compose for free.** The per-tile nondestructive transform pipeline
  ([src/js/core/transforms.js](src/js/core/transforms.js)) already runs on a flattened note list, so
...
([src/js/audio/instrument.js](src/js/audio/instrument.js)), deliberately *monotimbral* — **one drum per lane,
...
[src/js/core/random.js](src/js/core/random.js).
...
**The cheap enabler: a real scale-mask library — BUILT (2026-07).** In [src/js/core/scales.js](src/js/core/scales.js)
...
transport cluster ([src/js/ui/inspector.js](src/js/ui/inspector.js), a "Tile Inspector" button in the tile-player top
...
- **Per-tile transforms** ([src/js/core/transforms.js](src/js/core/transforms.js)) live on the tile *instance*, are
  applied when the score is built ([src/js/main.js](src/js/main.js) `arrangementScore`), never change the
...
- **Instrument patches live on the lane** (`lane.patch`, resolved per note by `patchFor(laneId)`),
  and **mods** ([src/js/audio/mods.js](src/js/audio/mods.js)) are a per-lane, per-instrument thing that stays there.
...
pane, [src/js/ui/instrumentpane.js](src/js/ui/instrumentpane.js)) with an inherit-or-override choice per setting.
...
- **Phase A — extract the modeless-pane primitive — BUILT (2026-07-06)** *(enabler, no visible change)*:
  ...
  chrome is now [src/js/ui/panel.js](src/js/ui/panel.js) (`createPanel`); the Tile inspector is its first tenant
...
- **Phase B — patch identity on the lane + the editor's Save/Load — BUILT (2026-07-06)** *(the core
  value)*: the user-global patch store ([src/js/audio/patches.js](src/js/audio/patches.js), factory `Init` per kind + user
...
- **Phase C — the patch catalog window — BUILT (2026-07-06)** *(browse + apply + manage)*:
  [src/js/ui/catalog.js](src/js/ui/catalog.js), a panel.js tenant opened from the instrument pane — kind → patch
...
3. **The pure generator/transform pattern — already established.**
   [src/js/core/random.js](src/js/core/random.js) (generator) and [src/js/core/transforms.js](src/js/core/transforms.js)
...
feed consonant-interval **`biasTargets`** into the existing **Steer** `biasedPick` ([src/js/core/random.js](src/js/core/random.js))
...
- **Chosen direction: two separate export options, NO pitch bend.** The user finds pitch-bend /
  ...
  - **Scala (microtonal)** — the **degree IS the interchange unit, no pitch math**. Emit the
    same degree-MIDI **plus a generated `.scl` + `.kbm`**, zipped (reuse [src/js/export/zip.js](src/js/export/zip.js)),
...
- **Auto-hocket** — a generator/transform that fills the **rests/gaps of a reference pattern**
  ...
  *where the reference is silent*, and rhythmic complementarity biases onsets/durations into
  those holes. Rides the existing **Duration Bias** machinery in [src/js/core/random.js](src/js/core/random.js);
